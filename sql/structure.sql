...
-- this sp is used to change the process status from 1 (running) to 2 (dbupdate).
-- if status is different from 1 or record doesn't exist, it's an error condition
DROP PROCEDURE IF EXISTS setDbUpdateProcess;
DELIMITER //
CREATE PROCEDURE setDbUpdateProcess(in p_mode tinyint(4), out all_ok tinyint(1))
  BEGIN
    DECLARE e_mode tinyint(4);
    DECLARE e_status tinyint(4);
    DECLARE no_process_rows BOOLEAN;
    ...
    IF no_process_rows THEN
      SET all_ok = 0;
    END IF;
-- i found record with status running (it's ok)
    IF (e_status = 1) THEN
      UPDATE
        process_info
      SET status = 2
      ...
      SET all_ok = 1;
    ELSE
-- i found a recod with status idle
      SET all_ok = 0;
    END IF;
